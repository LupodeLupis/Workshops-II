...
namespace sict {
	...
	{
		// If hero has a name attached print otherwise print No Hero
		if (hero.name[0] != '\0') {
			os << hero.name;
		}
	...
	const Hero & operator*(const Hero & first, const Hero & second)
	{
		...
		// check until the Hero A and Hero B are alive and round does not reach the max (100).
		while (a.isAlive() && b.isAlive() && round <= max_round)
		{
			if (a.isAlive() && b.isAlive()) {
				// call the binary operator to substract the strengh of the Hero B from the health of Hero A ( the opposite in the following line).
				a -= b.attackStrength();
				b -= a.attackStrength();
			}
			round++;
